...
def count_for_month(hour_avg, is_lear) :     
    ...
    if is_lear == 1 :
        ...
        month = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 362]
    month[12]=len(hour_avg)-1
    month_avg = []
   #print(len(hour_avg))
    #print(hour_avg[362])
    for i in range(0,12):
...
def count_for_8_hours(index, days, hour, year, iteration):
    ...
    month_avg = count_for_month(hour_avg, is_lear)
...
def count_for_24_hours(index, days, hour, year, iteration) :
    ...
    month_avg = count_for_month(hour_max, is_lear)
              
    return month_avg


# count for NO2, SO2
def count_for_1_hour(index, days, hour, year, iteration):
    is_lear = 0
...
#            O3 = i
    ...
    output_csv.to_csv("AirPolution_Taiwan/" + repr(year) + "/" + country + "/" + repr(year) + "_" + station + ".csv", encoding = 'gbk')
